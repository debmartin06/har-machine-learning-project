...
Before modeling, there's some data cleanup that needs to be done. First, we need to deal with NA values. There are many NA values in our model. For columns that are made up of 50% or more NA values, we will throw them out.
...
```
The accuracy using decision trees went down to 50% -- much less reliable than the model built using the Random Forest method. We will move forward with Random Forest.